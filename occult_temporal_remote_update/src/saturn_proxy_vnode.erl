...
async_update(Node, BKey, Value, CausalTimestamp, ClientNow, Client) ->
    riak_core_vnode_master:command(Node,
                                   {async_update, BKey, Value, CausalTimestamp, ClientNow, Client},
                                   {fsm, undefined, self()},
                                   ?PROXY_MASTER).
...
handle_command({update, BKey, Value, Deps}, _From, S0) ->
    {{ok, TimeStamp}, S1} = do_update(BKey, Value, Deps, 0, S0),
    {reply, {ok, TimeStamp}, S1};

handle_command({async_update, BKey, Value, CausalTimestamp, ClientNow, Client}, _From, S0) ->
    {{ok, ShardStamp}, S1} = do_update(BKey, Value, CausalTimestamp, ClientNow, S0),
...
do_update(BKey={Bucket, _}, Value, CausalTimestamp0, ClientNow, S0=#state{myid=MyId,
    ...
    case groups_manager:get_datanodes_ids(BKey, Manager#state_manager.groups, MyId) of
        {ok, Group} ->
            ...
            lists:foreach(fun(Id) ->
                            ...
                            UId = {CausalTimestamp1, ShardStamp1, MyId, ClientNow},
...
merge_new_update(UpdateCausalTimestamp, CausalTimestamp) ->
    merge_new_update(UpdateCausalTimestamp, CausalTimestamp, [], 0, []).

merge_new_update([], [], Result, _, _) ->
    Result;

merge_new_update([LocalEntry], [], Result, Size, _) ->
    case Size == ?VECTOR_SIZE of
        true ->
            Result;
        false ->
            Result ++ [LocalEntry]
...
merge_new_update([], [ReceivedEntry={_, Shard} | ReceivedRem], Result, Size, ExistingShards) ->
    case Size == ?VECTOR_SIZE of
        true ->
            Result;
        false ->
            ...
            case lists:member(Shard, ExistingShards) of
                true ->
                    merge_new_update([], ReceivedRem, Result, Size, ExistingShards);
                false ->
                    merge_new_update([], ReceivedRem, Result ++ [ReceivedEntry], Size + 1, ExistingShards ++ [Shard])
            end
    end;

merge_new_update(LocalTimestamp=[LocalEntry={LocalShardstamp, LocalShard}],
    ...
    case Size == ?VECTOR_SIZE of
        %% The vector reached the maximum size
        true ->
            Result;
        %% There is still some space to be filled
        false ->
            case ReceivedShardstamp > LocalShardstamp of
                true ->
                    ...
                    case lists:member(ReceivedShard, ExistingShards) of
                        true ->
                            merge_new_update(LocalTimestamp, ReceivedRem, Result, Size, ExistingShards);
                        false ->
                            merge_new_update(LocalTimestamp, ReceivedRem, Result ++ [ReceivedEntry], Size + 1, ExistingShards ++ [ReceivedShard])
                    end;
                false ->
                        merge_new_update([], ReceivedTimestamp, Result ++ [LocalEntry], Size + 1, ExistingShards ++ [LocalShard])
            end
    end.
