-module(saturn_benchmarks_da_vanilla_cops_partial_force_ratio_v2).

-define(RESET_DEPS_UPDATE_FREQUENCY, 0).
-define(N_READS, 9).
...
run(update, KeyGen, ValueGen, S0) ->
    S1 = gather_deps(KeyGen, ValueGen, S0),
...
%% Operations
read(KeyGen, S0=#state{node=Node,
                       deps=Deps,
                       correlation=Correlation,
                       local_buckets=LocalBuckets,
                       ordered_latencies=OrderedLatencies,
...
update_keep_dep(KeyGen, ValueGen, S0=#state{node=Node,
    ...
    {ok, Bucket} = pick_bucket_partial_replication(BucketFullReplication, S0),
    BKey = {Bucket, KeyGen()},
    ...
    case Result of
        {ok, Clock} ->
            {ok, S0#state{deps=insert_dep({BKey, Clock}, Deps)}};
        Else ->
            {error, Else}
    end.
...
update_reset(KeyGen, ValueGen, S0=#state{node=Node,
    ...
    %% Pick BKey
    BucketFullReplication = hd(LocalBuckets),
    BKey = {BucketFullReplication, KeyGen()},

    %% Make the request
    Result = gen_server:call(server_name(Node), {update, BKey, ValueGen(), dict:to_list(Deps)}, infinity),
    case Result of
        {ok, Clock} ->
            {ok, S0#state{deps=insert_dep({BKey, Clock}, dict:new())}};
        Else ->
            {error, Else}
    end.
...
gather_deps(KeyGen, ValueGen, NUpdates, NReads, S0) ->
    ...
    case NReads < ?N_READS of
        true ->
            {ok, S1} = read(KeyGen, S0),
            gather_deps(KeyGen, ValueGen, NUpdates, NReads + 1, S1);
        false ->
