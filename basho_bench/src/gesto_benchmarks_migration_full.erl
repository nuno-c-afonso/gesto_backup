-module(gesto_benchmarks_migration_full).
...
run(read, KeyGen, _ValueGen, #state{node=Node,
    ...
    case Result of
        {ok, {_Value, TimeStamp}} ->
            ...
            case LocalLabel0 of
                empty ->
                    {ok, S0#state{clock=Clock1, local_label={MyDc, BKey, TimeStamp}}};
                {_, _, Timestamp0} ->
        ...
        {ok, {_Value, TimeStamp, Sender, SerializerClock}} ->
            ...
            case RemoteLabel0 of
                empty ->
                    {ok, S0#state{clock=Clock1, remote_label={Sender, BKey, SerializerClock}}};
...
run(remote_read, KeyGen, _ValueGen, #state{nodes=Nodes,
    ...
    Result = gen_server:call(server_name(Node), {migration, LocalLabel0, RemoteLabel0}, infinity),
    case Result of
        {ok, SerializerClock} ->
            ...
            case LocalLabel0 of
                ...
                {Sender, BKey, _} ->
                    {ok, S0#state{node=Node,
                                    ...
                                    ordered_latencies=LatenciesOrdered,
                                    local_label=empty,
                                    remote_label={Sender, BKey, SerializerClock}}}
            end;
        Else ->
            {error, Else}
    end;
...
run(update, KeyGen, ValueGen, #state{node=Node,
    ...
    case Result of
        {ok, Clock1} ->
            {ok, S0#state{clock=Clock1, local_label={MyDc, BKey, Clock1}}};
        Else ->
            {error, Else}
    end;
