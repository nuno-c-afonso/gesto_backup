-module(saturn_benchmarks_da_migration_force_ratio).
...
%% TODO: Check if this function gives the right latencies. Some results are weird!!!
...
run(remote_read, KeyGen, _ValueGen, #state{node=Node,
    ...
    % New remote update result without double migration
    Result1 = gen_server:call(server_name(Node), {migrate, BKey, Clock0}, infinity),
    case Result1 of
        {ok, TimeStamp1} ->
