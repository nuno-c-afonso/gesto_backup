-module(saturn_benchmarks_da_vanilla_cops_partial_force_ratio).

%% 1- every turn; 2- one yes, one no; ...
-define(SAFE_UPDATE_FREQUENCY, 1).
...
run(read, KeyGen, _ValueGen, #state{node=Node,
    ...
    case Result of
        {ok, {_Value, TimeStamp}} ->
            Deps1 = insert_dep({BKey, TimeStamp}, Deps),
...
run(remote_read, KeyGen, _ValueGen, #state{deps=Deps,
                                            nodes=Nodes,
                                            correlation=Correlation,
                                            remote_buckets=RemoteBuckets,
                                            ordered_latencies=OrderedLatencies,
...
run(update, KeyGen, ValueGen, #state{node=Node,
                                     ...
                                     total_dcs=NumberDcs,
                                     local_buckets=LocalBuckets,
                                     n_updates=NrUpdates0}=S0) ->

    %% Manage the kind of update to apply
    NrUpdates1 = (NrUpdates0 + 1) rem ?SAFE_UPDATE_FREQUENCY,
    BucketFullReplication = hd(LocalBuckets),
    {ok, Bucket} = case NrUpdates1 of
        %% Make a safe update
        0 ->
            {ok, BucketFullReplication};
        %% Make other update
        _ ->
    ...
    BKey = {Bucket, KeyGen()},
    Result = gen_server:call(server_name(Node), {update, BKey, ValueGen(), dict:to_list(Deps)}, infinity),
    case Result of
        {ok, Clock} ->
            %% Changed for controlling the amount of dependencies, while maintaining the same local operation ratio
            Deps1 = case Bucket of
                %% Safe partition -> can reset the dependency list
                BucketFullReplication ->
                    insert_dep({BKey, Clock}, dict:new());
                %% Continue to add to existing dependency list
                _ ->
                    insert_dep({BKey, Clock}, Deps)
            end,

            {ok, S0#state{deps=Deps1, n_updates=NrUpdates1}};
        Else ->
            {error, Else}
    end;
