-module(saturn_benchmarks_da_migration_cops_fixed).
...
run(read, KeyGen, _ValueGen, #state{node=Node,
    ...
    case Result of
        {ok, {_Value, TimeStamp}} ->
            ...
            {ok, S0#state{clock=Clock1, deps=Deps1}};
        Else ->
            {error, Else}
    end;
...
run(remote_read, KeyGen, _ValueGen, #state{deps=Deps,
    ...
    Result = gen_server:call(server_name(Node), {read, BKey, dict:to_list(Deps), Clock0}, infinity),
    case Result of
        {ok, {_Value, TimeStamp}} ->
            Clock1 = max(TimeStamp, Clock0),
            Deps1 = insert_dep({BKey, TimeStamp}, Deps),
...
run(update, KeyGen, ValueGen, #state{node=Node,
    ...
    BKey = {Bucket, KeyGen()},
    Result = gen_server:call(server_name(Node), {update, BKey, ValueGen(), dict:to_list(Deps), Clock0}, infinity),
    case Result of
        {ok, Clock1} ->
            Deps1 = insert_dep({BKey, Clock1}, dict:new()),
            {ok, S0#state{clock=Clock1, deps=Deps1}};
        Else ->
            {error, Else}
    end;
