-module(saturn_benchmarks_occult_temporal_worst_gesto).
...
-define(VECTOR_SIZE, 20).       %% Maximum size of the causal timestamp
...
new(Id) ->
    ...
    ClosestReplicasFileName = basho_bench_config:get(closest_replica),
...
run(remote_read, KeyGen, ValueGen, S0=#state{all_buckets=AllBuckets}) ->
            ...
            case gen_server:call(server_name(Node), {read, BKey}, infinity) of
                {ok, {_Value, CausalTimeStampResponse, ShardStampResponse}} ->
                    ...
                    case (ShardStampResponse >= ShardStampClient) or lists:member(Bucket, LocalBuckets) of
                        ...
                        true ->
                            {ok, S0#state{causal_timestamp=merge_causal_timestamps(CausalTimestamp0, CausalTimeStampResponse)}};
    ...
    case Size == ?VECTOR_SIZE of
        ...
        false ->
            ...
            case lists:member(Shard, ExistingShards) of
                ...
                false ->
                    merge_causal_timestamps(LocalRem, [], Result ++ [LocalEntry], Size + 1, ExistingShards ++ [Shard])
            end
    end;
    ...
    case Size == ?VECTOR_SIZE of
        ...
        false ->
            ...
            case lists:member(Shard, ExistingShards) of
                ...
                false ->
                    merge_causal_timestamps([], ReceivedRem, Result ++ [ReceivedEntry], Size + 1, ExistingShards ++ [Shard])
            end
    end;
            ...
            case ReceivedShardstamp > LocalShardstamp of
                true ->
                    ...
                    case lists:member(ReceivedShard, ExistingShards) of
                        ...
                        false ->
                            merge_causal_timestamps(LocalTimestamp, ReceivedRem, Result ++ [ReceivedEntry], Size + 1, ExistingShards ++ [ReceivedShard])
                    end;
                false ->
                    %% Check if the shard already belongs to the vector in construction
                    case lists:member(LocalShard, ExistingShards) of
                        ...
                        false ->
                            merge_causal_timestamps(LocalRem, ReceivedTimestamp, Result ++ [LocalEntry], Size + 1, ExistingShards ++ [LocalShard])
                    end
            end
    end.
...
%% Operations
update(KeyGen, ValueGen, S0=#state{all_buckets=AllBuckets}) ->
...
update_bkey(BKey={Bucket, _}, ValueGen, S0=#state{node=Node,
                                                  nodes=Nodes,
                                                  total_dcs=NumberDcs,
                                                  local_buckets=LocalBuckets,
                                                  remote_buckets=RemoteBuckets,
    ...
    Result = gen_server:call(server_name(UpdateNode), {update, BKey, ValueGen(), CausalTimestamp0}, infinity),
    case Result of
