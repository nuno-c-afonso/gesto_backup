-module(saturn_benchmarks_da_migration_eventual_foreign).
...
run(remote_read, KeyGen, _ValueGen, #state{node=Node,
                                            ...
                                            mydc=MyDc,
                                            firstdc=FirstDc}=S0) ->
    Id = get_random_dc_id(FirstDc, MyDc, NumberDcs),
    BKey = {Id, KeyGen()},
    Node1 = get_node_from_id(Id, NumberDcs, Nodes),
    Result = gen_server:call(server_name(Node1), {read, BKey, Clock0}, infinity),
...
%% Start of get_random_dc_id/3
get_random_dc_id(FirstDc, MyDc, NumberDcs) ->
    get_random_dc_id_tail(FirstDc, MyDc, NumberDcs, random:uniform(NumberDcs) - 1).

get_random_dc_id_tail(FirstDc, MyDc, NumberDcs, NewDc) ->
    case (MyDc == NewDc) or (FirstDc == NewDc) of
        true ->
            get_random_dc_id_tail(FirstDc, MyDc, NumberDcs, random:uniform(NumberDcs) - 1);
        false ->
            NewDc
    end.
%% End of get_random_dc_id/3
