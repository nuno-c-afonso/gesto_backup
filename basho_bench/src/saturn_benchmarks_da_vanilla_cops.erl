-module(saturn_benchmarks_da_vanilla_cops).
...
run(update, KeyGen, ValueGen, #state{node=Node,
    ...
    case Result of
        {ok, Clock} ->
            Deps1 = insert_dep({BKey, Clock}, dict:new()),

            %% Just for the latency for the amount of dependencies test
            % timer:sleep(10), %% in milliseconds
            % timer:sleep(1), %% in milliseconds
