-module(saturn_benchmarks_da_occult_experiment).
...
-record(state, {node,
                ...
                %% FIXME: For now, this will have as many entries as the amount of shards.
                %% In the future, we need to add a compression technique.
...
update_bkey(BKey={Bucket, _}, ValueGen, S0=#state{node=Node,
    ...
    Result = gen_server:call(server_name(UpdateNode), {update, BKey, ValueGen(), dict:to_list(CausalTimestamp0)}, infinity),
    case Result of
