-module(gesto_benchmarks_migration_partial_free_concurrent_remote_updates).
...
run(read, KeyGen, _ValueGen, #state{node=Node,
    ...
    BKey = {Bucket, KeyGen()},
    S1 = S0#state{deps=sets:add_element(BKey, Dependencies0)},
    ...
    case Result of
        {ok, {_Value, TimeStamp}} ->
            ...
            case LocalLabel0 of
                empty ->
                    {ok, S1#state{clock=Clock1, local_label={MyDc, Bucket, TimeStamp}}};
                {_, _, Timestamp0} ->
                    case TimeStamp > Timestamp0 of
                        true ->
                            {ok, S1#state{clock=Clock1, local_label={MyDc, Bucket, TimeStamp}}};
                        false ->
                            {ok, S1#state{clock=Clock1}}
                    end
            end;
        {ok, {_Value, TimeStamp, Sender, SerializerClock}} ->
            ...
            case RemoteLabel0 of
                empty ->
                    {ok, S1#state{clock=Clock1, remote_label={Sender, Bucket, SerializerClock}}};
...
run(update, KeyGen, ValueGen, #state{node=Node,
    ...
    BKey = {Bucket, KeyGen()},
    NewDependencies = sets:new(),
    S1 = S0#state{deps=sets:add_element(BKey, NewDependencies)},
    NrDeps = sets:size(Dependencies),
    Result = gen_server:call(server_name(Node), {update, BKey, ValueGen(), Clock0, NrDeps}, infinity),
    case Result of
        {ok, Clock1} ->
            {ok, S1#state{clock=Clock1, local_label={MyDc, Bucket, Clock1}}};
        Else ->
            {error, Else}
    end;
