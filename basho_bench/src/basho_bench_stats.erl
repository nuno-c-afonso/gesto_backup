...
-record(state, { ops,
                 ...
                 %% Added for gesto-benchmarks
                 remote_read_latencies = []
                 }).
...
op_complete(Op, {ok, Units}, ElapsedUs) ->
   ...
   % io:format("Get distributed: ~p~n", [get_distributed()]),
    ...
    %% Adapted for gesto-benchmarks
    case Op of
        {remote_read, remote_read} ->
            gen_server:cast(?MODULE, {migration, ElapsedUs});
        _ ->
            noop
    end,
...
handle_cast({migration, ElapsedUs}, State=#state{remote_read_latencies = RemoteLatencies0}) ->
    {noreply, State#state{remote_read_latencies = RemoteLatencies0 ++ [ElapsedUs]}};
...
%% Adapted for gesto-benchmarks
report_latency(State=#state{stats_writer=Module,
                      remote_read_latencies=RemoteLatencies},
               Elapsed, Window, Op) ->
