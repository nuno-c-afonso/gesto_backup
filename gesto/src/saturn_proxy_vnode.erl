...
propagate(Node, TimeStamp, OriginNode, Sender, SerializerClock) ->
    riak_core_vnode_master:command(Node,
                                   {propagate, TimeStamp, OriginNode, Sender, SerializerClock},
                                   {fsm, undefines, self()},
                                   ?PROXY_MASTER).

remote_read(Node, Label, SerializerClock) ->
    riak_core_vnode_master:command(Node,
                                   {remote_read, Label, SerializerClock},
                                   {fsm, undefines, self()},
                                   ?PROXY_MASTER).
...
handle_command({data, Id, BKey, Value}, _From, S0=#state{data=Data,
    ...
    case Pending of
        {Id, Sender, SerializerClock} ->
            {TimeStamp, _} = Id,
            {Connector1, Staleness1} = do_remote_update(BKey, Value, TimeStamp, Sender, MyId, Connector0, Staleness0, SerializerClock),
...
handle_command({propagate, TimeStamp, Node, Sender, SerializerClock}, _From, S0=#state{connector=Connector0, myid=MyId, data=Data, staleness=Staleness0}) ->
    Id = {TimeStamp, Node},
    case ets:lookup(Data, Id) of
        [] ->
            {noreply, S0#state{pending={Id, Sender, SerializerClock}}};
        [{Id, {BKey, Value}}] ->
            {Connector1, Staleness1} = do_remote_update(BKey, Value, TimeStamp, Sender, MyId, Connector0, Staleness0, SerializerClock),
...
do_read(_, BKey, _, _, #state{myid=MyId, connector=Connector, manager=Manager}) ->
...
do_remote_update(BKey, Value, TimeStamp, Sender, MyId, Connector0, Staleness0, SerializerClock) ->
    Staleness1 = ?STALENESS:add_update(Staleness0, Sender, TimeStamp),
    Clock = case ?BACKEND_CONNECTOR:read(Connector0, {BKey}) of
        {ok, {_, UpdateTimeStamp, _, _}} ->
            UpdateTimeStamp;
        {ok, {_, UpdateTimeStamp}} ->
            UpdateTimeStamp
    end,
    case (Clock<TimeStamp) of
        true ->
            {ok, Connector1} = ?BACKEND_CONNECTOR:update(Connector0, {BKey, Value, TimeStamp, Sender, SerializerClock}),
