...
%% Call for creating the label with partial replication
migrate(MyId, Id) ->
    gen_server:cast({global, reg_name(MyId)}, {migrate, Id}).
...
%% Propagate migration label to the destination replica
handle_cast({migrate, Id}, S0=#state{labels=Labels, myid=MyId, vclock=VClock, manager=Manager}) ->
    TimeStamp = saturn_utilities:now_microsec(),
    Partition = -1,
    Time = 0,
    Label = create_label(TimeStamp, MyId, Id),
    ets:insert(Labels, {{TimeStamp, Time, Partition, Label}, in}),
    ok = deliver_labels(Labels, compute_stable(VClock), MyId, [], Manager),
    {noreply, S0};
...
%% For creating the migration labels
create_label(TimeStamp, MyId, Id) ->
    #label{operation=remote_read,
           bkey=none,
           timestamp=TimeStamp,
           node={-1, node()},
           sender=MyId,
           payload=#payload_remote{to=Id}}.
