...
do_update(BKey={Bucket, _}, Value, CausalTimestamp0, ClientNow, S0=#state{myid=MyId,
    ...
    %% Get the current shardstamp and increment it
    ShardStamp0 = case dict:is_key(Bucket, ShardStampDict0) of
        true ->
            dict:fetch(Bucket, ShardStampDict0);
        false ->
            0
    end,
    ShardStamp1 = ShardStamp0 + 1,

    %% Update the causal timestamp
    CausalTimestamp1 = lists:foldl( fun(ShardStampTemp={Shard, _}, CausalTimeStampTemp) ->
        case Shard == Bucket of
            true ->
                CausalTimeStampTemp;
            false ->
                CausalTimeStampTemp ++ [ShardStampTemp]
        end
    end, [{Bucket, ShardStamp1}], CausalTimestamp0),
