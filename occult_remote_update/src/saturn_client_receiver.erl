...
%% There will just be a single read operation
handle_call({read, BKey}, From, S0) ->
    saturn_leaf:async_read(BKey, From),
    {noreply, S0};

%% It is always made in the master of the shard
handle_call({update, BKey, Value, CausalTimestamp, ClientNow}, From, S0) ->
    saturn_leaf:async_update(BKey, Value, CausalTimestamp, ClientNow, From),
    {noreply, S0}.
