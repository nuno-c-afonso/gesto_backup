...
async_update({Bucket, Key}, Value, CausalTimestamp, ClientNow, Client) ->
    DocIdx = riak_core_util:chash_key({Bucket, none}),
    PrefList = riak_core_apl:get_primary_apl(DocIdx, 1, ?PROXY_SERVICE),
    [{IndexNode, _Type}] = PrefList,
    saturn_proxy_vnode:async_update(IndexNode, {Bucket, Key}, Value, CausalTimestamp, ClientNow, Client).
...
collect_stats(From, Type) ->
    ...
    FinalStats = case FinalStatsRaw of
        [] ->
            [];
        _ ->
            ?STALENESS:compute_cdf_from_orddict(FinalStatsRaw)
    end,
