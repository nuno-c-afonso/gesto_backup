...
migrate(Node, BKey, Clock, Client) ->
    riak_core_vnode_master:command(Node,
                                   {migrate, BKey, Clock, Client},
                                   {fsm, undefined, self()},
                                   ?PROXY_MASTER).
...
handle_command({migrate, BKey, Clock, Client}, _From, S0) ->
    {Res, S1} = do_migration(BKey, Clock, Client, S0),
    gen_server:reply(Client, {ok, Res}),
    {noreply, S1};
...
do_migration(BKey={Id, _}, Clock, From, S0=#state{myid=MyId, max_ts=MaxTS0, partition=Partition}) ->
    PhysicalClock = saturn_utilities:now_microsec(),
    TimeStamp = max(Clock, max(PhysicalClock, MaxTS0)),
    Label = create_label(remote_read, BKey, TimeStamp, {Partition, node()}, MyId, #payload_remote{to=Id, client=From, type_call=async}),
    saturn_leaf_producer:new_label(MyId, Label, Partition, false),    
    {TimeStamp, S0#state{max_ts=TimeStamp, last_label=Label}}.
