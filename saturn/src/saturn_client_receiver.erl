...
handle_call({migrate, BKey, Clock}, From, S0) ->
    saturn_leaf:migrate(BKey, Clock, From),
    {noreply, S0};

%% Read after the client migration
handle_call({read, BKey, IdOrigin, TimestampLabel, MyId}, From, S0) ->
    saturn_leaf:async_read(BKey, IdOrigin, TimestampLabel, From, MyId),
    {noreply, S0};
