...
async_read(BKey, IdOrigin, TimestampLabel, Client, MyId) ->
    saturn_leaf_converger:check_migration_label(MyId, BKey, IdOrigin, TimestampLabel, Client).

migrate(BKey, Clock, Client) ->
    DocIdx = riak_core_util:chash_key(BKey),
    PrefList = riak_core_apl:get_primary_apl(DocIdx, 1, ?PROXY_SERVICE),
    [{IndexNode, _Type}] = PrefList,
    saturn_proxy_vnode:migrate(IndexNode, BKey, Clock, Client).
